...
function planetOverviewDetails(planetName,callback){
  let findPlanet = false;
  let planetVal;
    for(let i=0; i<obj.length; i++){
        if(planetName == (obj[i].name)){
          findPlanet = true;
          planetVal = {
            name:obj[i].name,
            content:obj[i].overview.content,
            source: obj[i].overview.source,
            rotation:obj[i].rotation,
            revolution:obj[i].revolution,
            radius:obj[i].radius,
            temperature:obj[i].temperature,
            images:obj[i].images.planet
          }
          break;
        }
      }
      if(!findPlanet){
        ...
        callback(undefined, planetVal);
      }
      
}

function planetStructureDetails(planetName,callback){
  let planetfind = false;
  let planetValue;
    for(let i=0; i<obj.length; i++){
        if(planetName == (obj[i].name)){
          planetfind = true;
          planetValue = {
            name:obj[i].name,
            content:obj[i].structure.content,
            source:obj[i].structure.source,
            rotation:obj[i].rotation,
            revolution:obj[i].revolution,
            radius:obj[i].radius,
            temperature:obj[i].temperature,
            images:obj[i].images.internal
          }
          break;
        }
      }
      if(!planetfind){
        ...
        callback(undefined, planetValue);
      }
}

function planetGeologyDetails(planetName,callback){
  let findOfPlanet = false;
  let geologyPlanetVal;
    for(let i=0; i<obj.length; i++){
        if(planetName == (obj[i].name)){
          findOfPlanet = true;
          geologyPlanetVal = {
            name:obj[i].name,
            content:obj[i].geology.content,
            source: obj[i].geology.source,
            rotation:obj[i].rotation,
            revolution:obj[i].revolution,
            radius:obj[i].radius,
            temperature:obj[i].temperature,
            images:obj[i].images.geology
          }
          break;
        }
      }
      if(!findOfPlanet){
        ...
        callback(undefined, geologyPlanetVal);
      }
...
module.exports = ({
  planetOverviewDetails: planetOverviewDetails, 
  planetStructureDetails: planetStructureDetails,
  planetGeologyDetails: planetGeologyDetails,
});
...
// let mercuryPlanet = document.getElementById("mercury");
// let venusPlanet = document.getElementById("venus");
// let earthPlanet = document.getElementById("earth");
// let marsPlanet = document.getElementById("mars");
// let jupitorPlanet = document.getElementById("jupiter");
// let saturnPlanet = document.getElementById("saturn");
// let uranusPlanet = document.getElementById("uranus");
// let neptunePlanet = document.getElementById("neptune");

// let mercuryVal = (mercuryPlanet.textContent).toLowerCase();
// mercuryPlanet.addEventListener("click", async () => {
//     let p1 = await fetch("http://localhost:3300/planet?planetName="+mercuryVal).then((res)=> res.json());
//     console.log(p1);
// })
